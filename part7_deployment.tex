...
\textbf{Unified wire protocol.} All communication uses a compact binary protocol over WebSocket. DPF, HarmonyPIR, and OnionPIR each have their own request variants within the same framing, and each request optionally carries a trailing database identifier byte for multi-database routing (Section~\ref{sec:deployment-catalog}).
...
\begin{itemize}[nolistsep,leftmargin=*]
    \item \texttt{harmonypir/}: HMR12 (swap-or-not) backend---smallest binary, no SIMD requirement.
    \item \texttt{harmonypir-fastprp/}: FastPRP backend---precomputed permutation tables.
    \item \texttt{harmonypir-alf/}: ALF backend with WASM SIMD128---fastest in browsers with SIMD support ($4$--$8\times$ over HMR12).
\end{itemize}
...
\textbf{Catalog endpoint.} A dedicated catalog request returns the full list of databases currently served. Each catalog entry contains the \texttt{db\_id}, type, base height, tip height, cuckoo parameters (index and chunk bin counts, tag seed), and a flag indicating whether per-group Merkle verification data is available for that database. The top-level server metadata response additionally embeds per-database Merkle metadata (arity, level sizes, per-group roots, super-root, and tree-top hash), so that a client can verify Merkle proofs against the correct database without a second round trip.
...
\subsection{Database Authenticity and Bitcoin Anchoring}
\label{sec:database-authenticity}

Merkle verification proves that a PIR answer is included in the database roots served for a particular database. A separate authenticity chain connects those roots to Bitcoin history. The client or auditor checks the chain in stages: the PIR answer opens to a database Merkle super-root; the super-root is named by an attested database build proof; the build proof names a snapshot height, block hash, and MuHash; and a separate blockhash-to-MuHash proof shows that the same snapshot tuple appears under an SEV-SNP-attested MuHash tree root.

The database proof is the bridge from BitcoinPIR's served artifacts to a Bitcoin snapshot anchor. The builder emits the bucket and OnionPIR Merkle super-roots for the database structures used by the PIR backends. Its proof records the snapshot height, Bitcoin block hash, and Bitcoin Core-display MuHash value, and binds those fields to the build evidence and SNP \texttt{REPORT\_DATA}. A client or auditor can therefore check that the database roots used for query verification are the same roots named by the database proof.

The blockhash-to-MuHash proof, abbreviated BHTM in the implementation notes, checks the complementary statement: the same \((\text{height}, \text{block hash}, \text{MuHash})\) tuple appears in an append-only MuHash tree. The BHTM runner is intentionally narrow. It streams stripped block deltas prepared next to Bitcoin Core, recomputes each 80-byte header hash, checks height continuity and previous-header linkage, recomputes the transaction-id Merkle root from stripped transaction bytes, applies supplied Bitcoin Core-style coin additions and removals to a MuHash accumulator, and appends \(\text{height} \,\|\, \text{block hash} \,\|\, \text{full 384-byte MuHash}\) as a tree leaf. The chunk tree root is committed by an AMD SEV-SNP quote over a reusable BHTM proof-runner image.

The current checked witness ties the published database proof at height \(948{,}454\) to the BHTM proof for the \(900{,}000\)--\(954{,}920\) chunk. The database proof and BHTM leaf proof agree on the block hash with prefix \(0000000000000000\) and suffix \(5560a4e4\), and on the Core-display MuHash with prefix \(cf4fc1f1dd400622\) and suffix \(56c2a2ee\). The accepted database roots have bucket-root prefix \(e2ba2eee67884243\) and OnionPIR-root prefix \(f86baa3966a61cdc\). The BHTM leaf proof has leaf index \(48{,}454\), tree size \(54{,}921\), 16 Merkle proof steps, and tree-root prefix \(babeea635812c3b1\). All archived chunks from \(0\)--\(99{,}999\) through the partial current-head chunk \(900{,}000\)--\(954{,}920\) have been proven with the same streaming UKI measurement, whose prefix is \(652f8c813382abb0\); for the current-head chunk, the BHTM quote and AMD VCEK/report-signature verification passed.

This is not a claim that the BHTM guest is a full Bitcoin consensus validator. The guest proves a narrower computation: given header-bound stripped transaction commitments and supplied Bitcoin Core-format coin deltas, the measured program computed the stated MuHash value and MuHash tree root. It does not validate proof of work, script or witness rules, coinbase maturity, subsidy, double spends, BIP30 behavior, genesis handling, or unspendable-output filtering. Those rules remain responsibilities of the data source and extractor. This split keeps the attested computation small and auditable while making the public claim precise: the PIR database roots are tied to an attested MuHash transition for a named Bitcoin block hash, under the stated data-source trust boundary.
