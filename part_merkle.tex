...
The super-root, together with the database version, is what clients compare across servers or against the database-authenticity anchor described in Section~\ref{sec:database-authenticity}. One 32-byte value covers every bin of every group at both levels.
...
\begin{enumerate}[nolistsep,leftmargin=*]
    \item \textbf{Anchor the tree-top cache.} On the first verification of a session, the client fetches the tree-top cache and computes its SHA-256. The expected hash is included in the top-level server metadata response (Section~\ref{sec:deployment-catalog}); if it matches, the client has a locally trusted copy of all per-group roots and of the super-root.
    ...
    \item \textbf{Walk the tree, one level at a time.} For each sibling level $\ell = 0, 1, 2, \ldots$, the client needs the $A - 1 = 7$ sibling hashes of its current node at that level. Because siblings are stored in the flat level-$\ell$ table indexed by (group, node position), the client already knows exactly which row of which file it needs to read---but reading that row in cleartext would reveal the query. Instead, the client treats each sibling table as just another PIR database and issues a batch DPF-PIR query against it, piggybacking on the regular 2-server DPF primitive.
...
\textbf{OnionPIR.} OnionPIR uses a separate per-bin Merkle design with two flat trees (one for the INDEX level, one for CHUNK). Leaf hashes are computed over decrypted bin contents, and sibling queries are issued through the same FHE primitive used for data retrieval. The underlying two-tree structure mirrors the two-level PIR architecture, and the privacy invariants (batch padding, per-query leaf-count symmetry) are preserved through the same mechanisms described above.
